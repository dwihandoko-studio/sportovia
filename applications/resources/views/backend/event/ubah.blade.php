...
<div class="widget-box">
  ...
  <div class="widget-content nopadding">
    <form class="form-horizontal" method="post" action="{{ route('event.edit') }}" name="basic_validate" id="basic_validate" novalidate="novalidate" enctype="multipart/form-data">
      {{ csrf_field() }}
      <div class="control-group {{ $errors->has('tanggal_event') ? 'error' : '' }}">
        <label class="control-label">Event Date</label>
        <div class="controls">
          <input type="hidden" name="id" value="{{ $get->id }}">
          <div data-date="{{ old('tanggal_event', $get->tanggal_event) }}" class="input-append date tanggal_event">
            <input type="text" name="tanggal_event" value="{{ old('tanggal_event', $get->tanggal_event) }}" data-date-format="yyyy-mm-dd" class="span9" readonly="">
      ...
      <div class="control-group {{ $errors->has('judul') ? 'error' : '' }}">
        ...
        <div class="controls">
          <input type="text" name="judul" class="span6" id="judul" value="{{ old('judul', $get->judul) }}" />
        </div>
      </div>
      <div class="control-group {{ $errors->has('deskripsi') ? 'error' : '' }}">
        <label class="control-label">Content *</label>
        <div class="controls">
          <textarea name="deskripsi" class="span6" id="deskripsi" cols="20" rows="20">{{ old('deskripsi', preg_replace('#<br\s*/?>#i', "\n", $get->deskripsi)) }}</textarea>
        </div>
      </div>
      <div class="control-group {{ $errors->has('img_banner') ? 'error' : '' }}">
        <label class="control-label">Main Image *</label>
        <div class="controls">
          <span>Width: 1020px; Height: 510px</span>
        </div>
        <div class="controls">
        ...
        <div class="controls">
          <img src="{{ asset('amadeo/images/news-event').'/'.$get->img_banner }}" alt="{{ $get->img_banner_alt }}">
        </div>
      </div>
      <div class="control-group {{ $errors->has('img_thumb') ? 'error' : '' }}">
        <label class="control-label">Thumbnail Image *</label>
        <div class="controls">
          <span>Width: 274px; Height: 500px</span>
        </div>
        <div class="controls">
        ...
        <div class="controls">
          <img src="{{ asset('amadeo/images/news-event').'/'.$get->img_thumb }}" alt="{{ $get->img_thumb_alt }}">
        </div>
      </div>
      <div class="control-group {{ $errors->has('tanggal_publish') ? 'error' : '' }}">
        <label class="control-label">Publish Date</label>
        <div class="controls">
          <div data-date="{{ old('tanggal_publish', $get->tanggal_publish) }}" class="input-append date datepicker">
